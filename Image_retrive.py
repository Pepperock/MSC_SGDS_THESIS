...
def street_topic(df, names,output_direct, api_key):
  api_key = api_key
  count = 0
  name = names
  path = output_direct
  ...
  for row in df.iterrows():
      ...
      heading1 = name + str(count)
      count += 1

      street_image = StreetViewer2(api_key= api_key,location=coordinate, heading = heading1, output_directory= path)
